...
function naiveComposeMiddleware(middlewares: any) {
  if (middlewares.length === 0)
    return (store: any) => (dispatch: any) => dispatch
...
function composeActionHandlers(actionHandlers: any) {
...
export function combineMiddlewareWithActionHandlers(...middlewares) {
  ...
  const optimizedMiddlewares: any = []
