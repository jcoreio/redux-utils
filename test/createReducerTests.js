...
describe('createReducer', () => {
  ...
  it("sets actionHandlers prop of returned function", () => {
    ...
    expect(createReducer('state', actionHandlers).actionHandlers).to.equal(actionHandlers)
  })
  ...
  it("returns state if no actionHandlers were provided", () => {
    ...
    expect(reducer("can't touch this", {type: 'test'})).to.equal("can't touch this")
  })
  it("applies correct action handler", () => {
    ...
    let reducer = createReducer({}, actionHandlers)
    let state
    expect(reducer(state, {type: 'action1', payload: 'test1'})).to.deep.equal({field1: 'test1'})
    expect(reducer(state, {type: 'action2', payload: 'test1'})).to.deep.equal({field2: 'test1'})
  })
})
